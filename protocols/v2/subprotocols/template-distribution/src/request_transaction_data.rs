...
/// Message used by an upstream(Usually Template Provider) to respond successfully to a
...
/// Message used by an upstream(Usually Template Provider) to respond with an error to a
/// [`RequestTransactionData`] message.
#[derive(Serialize, Deserialize, Debug, Clone, PartialEq, Eq)]
