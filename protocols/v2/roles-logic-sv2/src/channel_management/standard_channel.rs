...
impl StandardChannel {
    pub fn new(
        ...
        let job_creator = JobFactory::new(None, None); // standard jobs have extranonce_len = 0
        Self {
            channel_id,
            user_identity,
            extranonce_prefix,
    ...
    /// to the past job.
    ///
    /// And we return the `NewMiningJob` message that can be sent downstream.
    pub fn new_active_job(&mut self) -> Result<NewMiningJob<'static>, StandardChannelError> {
        let active_chain_tip = self
            .active_chain_tip
            .as_ref()
            .ok_or(StandardChannelError::NoActiveChainTip)?;
    ...
    pub fn new_future_job(&mut self) -> Result<NewMiningJob<'static>, StandardChannelError> {
        let job = self
            .job_factory
            .new_standard_job(self.channel_id, None, self.extranonce_prefix.clone())
            .map_err(|_| StandardChannelError::JobFactoryError)?;
