...
impl ExtendedChannel {
    ...
    /// to the past job.
    ///
    /// And we return the [`NewExtendedMiningJob`] message that can be sent downstream.
    pub fn new_active_job(
        &mut self,
    ) -> Result<NewExtendedMiningJob<'static>, ExtendedChannelError> {
